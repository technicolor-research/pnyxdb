...
func TestSet_Add(t *testing.T) {
	...
	require.Exactly(t, []byte{
		0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 'a', 'l', 'i', 'c', 'e',
		0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x41, 0x00, 0x42,
	}, s.raw)
}
...
func TestSet_Remove(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(c.name, func(t *testing.T) {
			s, _ := getTestSet()
			removed, err := s.Remove(c.data)
			require.Exactly(t, c.removedExpected, removed, "invalid removed value")
			...
			require.Exactly(t, c.rawExpected, s.raw, "wrong raw value")
			require.Exactly(t, c.elementsExpected, s.Elements, "wrong elements value")
		})
	}
}
